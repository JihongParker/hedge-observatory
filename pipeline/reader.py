"""observatory Stage 1-2 reader — papers/python/04_esg/derivative_parser.py에서 이식(2026-09-07).
원본은 380사 패널 구축에서 검증됨. 키는 env/~/.opendart_key 전용.
"""
"""
...
def find_annual(corp_code, bgn="20260101", end="20260630"):
    q = (f"{BASE}/list.json?crtfc_key={KEY}&corp_code={corp_code}"
         f"&bgn_de={bgn}&end_de={end}&pblntf_detail_ty=A001&page_count=20")
...
def note_profile(full):
    ...
    """
    types, units = set(), set()
    for m in re.finditer(r"<TABLE.*?</TABLE>", full, re.S | re.I):
        tb = m.group(0)
        pos = m.start()

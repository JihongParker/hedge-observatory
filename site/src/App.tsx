...
const TOUR: { module: string; title: string; body: string; target: string }[] = [
  { module: 'overview', title: '점 하나가 회사 하나', body: '하늘의 별 2,391개가 읽을 대상 회사들입니다. 밝게 빛나는 별은 시범으로 이미 읽은 곳입니다. 관측소가 별을 하나씩 읽고, 이 사이트는 그 결과만 보여 줍니다.', target: '.ov-hero' },
  ...
  { module: 'coverage', title: '문장 하나로 판정', body: 'S-Oil 보고서에는 위험회피라는 말이 12번 나오지만 전부 상투적 문구였고, "적용하지 않는다"는 문장 하나가 결론이었습니다. 그래서 단어 수를 세지 않고 문장을 읽어 판정합니다.', target: '[data-tour="firms"]' },
  { module: 'kssb', title: '다음 관측', body: '2027년부터 큰 회사들은 기후 정보를 의무로 공시합니다. 의무가 행동을 바꾸는지는 2030~31년에 이 표를 열어 보면 압니다. 따로 연구하지 않아도 조회 한 번이면 됩니다.', target: '[data-tour="window"]' },
]
    ...
    <div className="app">
      ...
      <nav className={navOpen ? 'sidebar open' : 'sidebar'}>
        <div className="sb-brand" onClick={() => go('overview')} role="button" tabIndex={0}
          ...
          <div><b>헤지 관측소</b><span>상장사 헤지 관측소</span></div>
        </div>
